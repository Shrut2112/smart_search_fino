...
from data_ingestion.universal_parser_agent import (
    init_worker,
    clean_text_fixed,
    semantic_chunking_production,
)
from database.db_hooks import (
    init_db_pool,
    get_db_connection,
    upsert_doc,
    upsert_chunks,
...
def run_crawl(crawler: FinoCrawler, single_url: str = None):
    ...
    for i, url in enumerate(urls, 1):
        ...
        log.info(f"[{i}/{len(urls)}] (ETA: {eta_str}) Scraping: {url}")
        ...
        log.info(f"[{i}/{len(urls)}] 📊 Running: {processed} new | {skipped} unchanged | {failed} failed")

    log.info(
        f"Pass 1 done: {processed} processed, {skipped} skipped, "
        f"{failed} failed, {len(urls)} total"
    )

    # ── Retry sweep for failed URLs ──
    if failed_urls:
        log.info(f"\n{'='*60}")
        log.info(f"RETRY SWEEP: {len(failed_urls)} failed URLs — cooling down 60s before retry...")
        log.info(f"{'='*60}")
        time.sleep(60)  # Let Firecrawl backend recover
        
        retry_success = 0
        retry_still_failed = 0
        
        for i, url in enumerate(failed_urls, 1):
            log.info(f"  [RETRY {i}/{len(failed_urls)}] Scraping: {url}")
            page = crawler.scrape_page(url)
            
            if not page:
                log.warning(f"  [RETRY {i}/{len(failed_urls)}] ❌ Still failed: {url}")
                retry_still_failed += 1
                continue
            
            if process_web_page(crawler, page["url"], page["markdown"], page["metadata"]):
                retry_success += 1
                log.info(f"  [RETRY {i}/{len(failed_urls)}] ✅ Recovered: {url}")
            else:
                skipped += 1
        
        log.info(f"\nRetry sweep results: {retry_success} recovered, {retry_still_failed} permanently failed")
        processed += retry_success
        failed = retry_still_failed

    log.info(
        f"\n{'='*60}\n"
        f"FINAL CRAWL REPORT:\n"
        f"  Total URLs:    {len(urls)}\n"
        f"  Processed:     {processed}\n"
        f"  Unchanged:     {skipped}\n"
        f"  Failed:        {failed}\n"
        f"  Total Time:    {(time.time()-crawl_start)/60:.1f}m\n"
        f"{'='*60}"
    )
