...
class FinoCrawler:
    ...
    def scrape_page(self, url: str, max_retries: int = 3) -> Optional[Dict]:
        ...
        Returns dict with 'markdown', 'metadata', 'url' or None on failure.
        
        Progressive strategy:
          Attempt 1: full SPA wait (10s), normal scrape
          Attempt 2: reduced wait (5s),  after 15s backoff
          Attempt 3: minimal wait (2s),  after 30s backoff
        """
        # Progressive wait times — shorter on retries to avoid Firecrawl timeout
        wait_times = [SPA_WAIT_MS, 5000, 2000]
        ...
        for attempt in range(max_retries):
            ...
            if attempt > 0:
                ...
                log.info(f"  ↻ Retry {attempt}/{max_retries-1} for {url} (backoff={sleep_s}s, wait_for={wait_ms}ms)")
                time.sleep(sleep_s)
            ...
            try:
                ...
                if page:
                    if attempt > 0:
                        log.info(f"  ✅ Retry succeeded on attempt {attempt+1} for {url}")
                    return page
                else:
                    log.warning(f"Empty/tiny content from {url} (attempt {attempt+1})")

            except Exception as e:
                log.warning(f"Scrape attempt {attempt+1}/{max_retries} failed for {url}: {e}")
        
        log.error(f"❌ All {max_retries} scrape attempts failed for {url}")
        return None
