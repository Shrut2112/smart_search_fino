...
semantic_cache_query = f"""
  ...
  WHERE id = (
    SELECT id 
    FROM cache 
    WHERE embedding_vector <=> %s::vector < 0.10 AND language = %s
...
"""
insert_cache = f"""
  INSERT INTO cache (query_hash, user_query, answer, embedding_vector,language, ask_count, last_asked, created_at)
