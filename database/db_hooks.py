...
def init_db_pool():
    ...
    if _db_pool is None:
        ...
        # Note: Using port 5432 session pooling as provided.
        _db_pool = psycopg2.pool.ThreadedConnectionPool(
            1, 10,
            dbname="postgres",
            user="postgres",
            password="Smart_search_fino",
            host="db.uamjyadmzdkrnukevezv.supabase.co",
            port=5432,
            sslmode="require"
        )
...
def get_db_connection():
    ...
    # Liveliness check: Supabase PgBouncer periodically closes idle connections
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT 1")
    except (psycopg2.OperationalError, psycopg2.InterfaceError) as e:
        log.warning("Stale database connection detected, recreating...")
